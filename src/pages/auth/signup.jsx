...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!name.trim() || !email || !password || !confirmPassword) { setError('All fields are required.'); return; }
    if (!isWorkEmail(email)) { setError('Please use your work email address — personal email providers like Gmail aren’t allowed.'); return; }
  ...
  return (
    ...
    >
      ...
      {/* Right panel */}
      <div className="flex-1 min-h-0 flex items-center justify-center overflow-y-auto px-6 py-[clamp(8px,1.5vh,40px)] sm:px-10 xl:px-16">
        <div className="w-full max-w-[371px] flex flex-col items-center">
          ...
          {/* Heading */}
          <div className="flex flex-col items-center gap-[clamp(2px,0.5vh,6px)] text-center mb-[clamp(6px,1.6vh,30px)] w-full">
            <h1
              className="text-md leading-[1.05] tracking-[-0.05em] xl:tracking-[-0.07em] text-[#121212] font-medium"
              style={{ fontFamily: "'Noto Serif Display', serif" }}
            >
              Create Account
            </h1>
            <p className="text-[clamp(10px,1.2vh,13px)] leading-5 text-[#3d3d3d]">
              Enter your details to set up your recruiter workspace
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="w-full flex flex-col gap-[clamp(12px,3.5vh,40px)]">
            <div className="flex flex-col gap-[clamp(10px,2.2vh,24px)]">
              ...
              <div className="flex flex-col gap-[clamp(4px,0.8vh,8px)]">
                <Label htmlFor="name" className="text-[clamp(12px,1.8vh,16px)] font-normal leading-6 text-[#121212]">Full Name</Label>
                <Input
                  id="name"
                  type="text"
                  value={name}
              ...
              {/* Work Email */}
              <div className="flex flex-col gap-[clamp(4px,0.8vh,8px)]">
                <Label htmlFor="email" className="text-[clamp(12px,1.8vh,16px)] font-normal leading-6 text-[#121212]">Work Email</Label>
                <Input
                  id="email"
                  type="email"
                  value={email}
              ...
              {/* Confirm Password */}
              <div className="flex flex-col gap-[clamp(4px,0.8vh,8px)]">
                <Label htmlFor="confirmPassword" className="text-[clamp(12px,1.8vh,16px)] font-normal leading-6 text-[#121212]">Confirm Password</Label>
                <div className="relative">
                  <Input
                    id="confirmPassword"
          ...
          <p className="mt-[clamp(12px,3vh,32px)] text-[clamp(12px,1.8vh,16px)] leading-6 text-[#3e3e3e]">
            ...
            <Link to="/login" className="font-medium text-[#121212] hover:underline">
              Sign In
            </Link>
          </p>
        </div>
